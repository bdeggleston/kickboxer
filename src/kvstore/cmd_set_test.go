...
func TestSet(t *testing.T) {
	...
	ts := time.Now()
	rawval, err := r.ExecuteInstruction(store.NewInstruction("SET", "a", []string{"b"}, ts))
	if err != nil {
...
func TestSetValidation(t *testing.T) {
	...
	val, err = r.ExecuteInstruction(store.NewInstruction("SET", "a", []string{"x", "y"}, time.Now()))
	...
	val, err = r.ExecuteInstruction(store.NewInstruction("SET", "a", []string{"x"}, time.Time{}))
	if val != nil { t.Errorf("Expected nil value, got %v", val) }
	if err == nil {
		t.Errorf("Expected error, got nil")
	} else {
